...
# --- Color Palette (HARMONIC insight 標準) ---
...
add_textbox(slide, Inches(2), Inches(3.8), Inches(9.3), Inches(0.7),
            "令和6年法律第49号（令和6年6月14日公布 / 令和7年12月12日 完全施行）",
...
schedules = [
    ("第1段階", "令和6年9月1日施行", "施行済み"),
    ("第2段階", "令和6年12月13日施行", "施行済み"),
    ("第3段階", "令和7年12月12日施行", "完全施行済み"),
]
...
この改正は令和6年6月に公布され、3段階に分けて施行されてきました。
そして令和7年12月12日、つまり2025年12月に第3段階が施行され、すべての規定が完全施行となっています。
「もう始まっている」法律です。「うちはまだ対応してない」では済まない状況ですので、
何が変わったのか、何をすべきなのか、一緒に確認していきましょう。
...
add_notes(slide, """では、そもそもなぜこの改正が必要だったのか。背景を押さえておきましょう。
...
2025年12月の完全施行で、3つの柱すべてが法的効力を持っています。
...
ポイントは3つあります。
1つ目、建設業者に対して「労働者の処遇を確保すること」が努力義務になっています。
これまでも当然やるべきことでしたが、法律でちゃんと位置づけられたのは大きいですね。
2つ目、中央建設業審議会が「労務費の基準」を作成しました。
これによって「著しく低い労務費」での見積りの依頼や提出が禁止されています。
要するに、「安くしてよ」と無理なお願いをすること自体がNGになったわけです。
3つ目、不当に低い請負代金の禁止が、発注者だけでなく受注者にも広がりました。
これは2025年12月の第3段階で施行された規定です。
下請けに丸投げで安く叩く、というのも許されません。

詳しくは国交省のガイドラインをご確認ください。
https://www.mlit.go.jp/tochi_fudousan_kensetsugyo/const/tochi_fudousan_kensetsugyo_const_tk1_000001_00033.html""")
...
注文者、つまり発注側が「著しく低い労務費」で見積りを依頼すると、これは法律違反です。
同じように、受注者が自ら低すぎる労務費で見積りを出すことも禁止されています。
「著しく低い」の基準は、中央建設業審議会が「労務費に関する基準」として作成・公表しています。
...
まず「働き方改革」のほうから。
目玉は、「著しく短い工期」での契約の禁止です。
いわゆる「工期ダンピング」ですね。無理な工期で受けて、現場に長時間労働を強いる。
これが法律ではっきりNGになっています。
2025年12月の完全施行で、受注者側の工期ダンピングも禁止対象に加わりました。
つまり、無理な工期で「受ける」こと自体も違反です。
注文者にも工期に関する責任が課され、違反した建設業者には指導・監督が入ります。
また、資材が手に入らないなどの事情で工期の変更が必要なときは、協議をスムーズに行う制度も導入されています。
次に「生産性向上」のほう。
...
# フッターメッセージ
footer_box = add_shape_bg(slide, Inches(0.8), Inches(7.05), Inches(11.7), Inches(0.35),
                          COLOR_HIGHLIGHT_BG, 0.5)
add_textbox(slide, Inches(1.0), Inches(7.07), Inches(11.3), Inches(0.3),
            "出典：国土交通省「建設業法・入契法改正（令和6年法律第49号）について」  https://www.mlit.go.jp/tochi_fudousan_kensetsugyo/const/tochi_fudousan_kensetsugyo_const_tk1_000001_00033.html",
            font_size=9, color=COLOR_GRAY, alignment=PP_ALIGN.CENTER)

add_notes(slide, """最後に、建設業者の皆さんが具体的に何をすべきか、4つのポイントにまとめました。
すでに完全施行されていますので、「これからやる」ではなく「今すぐ確認」のつもりで聞いてください。
1つ目、処遇改善への対応。労務費の基準はすでに公表されています。
自社の見積りが基準を下回っていないか、しっかりチェックしてください。
2つ目、契約書の見直し。請負代金の変更方法が契約書に入っていますか？
おそれ情報の通知体制も整えておきましょう。根拠資料の収集・保存の仕組みも重要です。
3つ目、工期の適正化。無理な工期での受注は法律違反です。受ける側も、出す側も。
週休2日の確保や長時間労働の是正にも、積極的に取り組んでいきましょう。
4つ目、ICT活用の検討。タブレットやカメラの導入で、技術者の専任義務が緩和されます。
人手不足への対策としても、ぜひ検討してみてください。
以上が、建設業法・入契法改正のポイントでした。
この改正は、建設業で働く皆さんの処遇を守り、持続可能な業界にしていくための大きな一歩です。
まだ対応が済んでいない項目があれば、今日から始めましょう。ご視聴ありがとうございました。
